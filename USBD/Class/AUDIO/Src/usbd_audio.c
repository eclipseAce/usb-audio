...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  0x00,                                 /* bAssocTerminal */
  0x02,                                 /* bNrChannels */
  0x03,                                 /* wChannelConfig 0x0003  Mono */
  0x00,
  0x00,                                 /* iChannelNames */
  0x00,                                 /* iTerminal */
  /* 12 byte*/
  ...
  0x02,                                 /* bNrChannels */
  AUDIO_OUT_SUBFRAME,                   /* bSubFrameSize :  2 Bytes per frame (16bits) */
  USBD_AUDIO_BITRES,                    /* bBitResolution (16-bits per sample) */
  ...
  AUDIO_EPOUT_ADDR,                     /* bEndpointAddress 1 out endpoint */
  0x05,                                 /* bmAttributes */
  AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*2(Stereo)*2(HalfWord)) */
  0x01,                                 /* bInterval */
  0x00,                                 /* bRefresh */
  AUDIO_EPIN_ADDR,                      /* bSynchAddress */
  /* 09 byte*/
  ...
  AUDIO_STANDARD_ENDPOINT_DESC_SIZE,    /* bLength */
  USB_DESC_TYPE_ENDPOINT,               /* bDescriptorType */
  AUDIO_EPIN_ADDR,                      /* bEndpointAddress 1 out endpoint */
  0x01,                                 /* bmAttributes */
  AUDIO_PACKET_SZE(USBD_AUDIO_FREQ),    /* wMaxPacketSize in Bytes (Freq(Samples)*2(Stereo)*2(HalfWord)) */
  0x01,                                 /* bInterval */
  0x02,                                 /* bRefresh */
...
static int16_t USBD_AUDIO_ApplyVolumeControl(int16_t sample, int16_t volume) {
  ...
  if (volume < AUDIO_VOL_MIN) {
    volume = AUDIO_VOL_MIN;
  }
  /* Get number of shifts, round half-up */
  int16_t shifts = (AUDIO_VOL_MAX - volume + (AUDIO_VOL_RES / 2)) / AUDIO_VOL_RES;
...
/**
  ...
  */
static USBD_DescHeaderTypeDef *USBD_FindDesc(uint8_t *buf, uint8_t type, uint8_t subType)
{
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
  ...
  /* Open EP OUT */
  USBD_LL_OpenEP(pdev, AUDIO_EPOUT_ADDR, USBD_EP_TYPE_ISOC, AUDIO_OUT_PACKET);
  pdev->ep_out[AUDIO_EPOUT_ADDR & 0xFU].is_used = 1U;
  pdev->ep_out[AUDIO_EPOUT_ADDR & 0xFU].bInterval = 1U;

  /* Open EP IN */
  USBD_LL_OpenEP(pdev, AUDIO_EPIN_ADDR, USBD_EP_TYPE_ISOC, 3);
  pdev->ep_in[AUDIO_EPIN_ADDR & 0xFU].is_used = 1U;
  pdev->ep_in[AUDIO_EPIN_ADDR & 0xFU].bInterval = 1U;

  /* Flush feedback endpoint */
  USBD_LL_FlushEP(pdev, AUDIO_EPIN_ADDR);
  ...
  USBD_LL_PrepareReceive(pdev, AUDIO_EPOUT_ADDR, haudio->buffer, AUDIO_OUT_PACKET);
...
static uint8_t USBD_AUDIO_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx) {
  ...
  /* Flush all endpoints */
  USBD_LL_FlushEP(pdev, AUDIO_EPOUT_ADDR);
  USBD_LL_FlushEP(pdev, AUDIO_EPIN_ADDR);

  /* Close EP OUT */
  USBD_LL_CloseEP(pdev, AUDIO_EPOUT_ADDR);
  pdev->ep_out[AUDIO_EPOUT_ADDR & 0xFU].is_used = 0U;
  pdev->ep_out[AUDIO_EPOUT_ADDR & 0xFU].bInterval = 0U;

  /* Close EP IN */
  USBD_LL_CloseEP(pdev, AUDIO_EPIN_ADDR);
  pdev->ep_in[AUDIO_EPIN_ADDR & 0xFU].is_used = 0U;
  pdev->ep_in[AUDIO_EPIN_ADDR & 0xFU].bInterval = 0U;
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
  ...
  if (epnum == AUDIO_EPOUT_ADDR) {
    ...
    USBD_LL_PrepareReceive(pdev, AUDIO_EPOUT_ADDR, &haudio->buffer[haudio->wr_ptr], AUDIO_OUT_PACKET);
  }
...
static uint8_t USBD_AUDIO_SOF(USBD_HandleTypeDef *pdev) {
  ...
  if (fnsof_interval > 4) {
    ...
    USBD_LL_Transmit(pdev, AUDIO_EPIN_ADDR, (uint8_t *)&haudio->fb_value, 3U);
  }
 ...
 */
static uint8_t USBD_AUDIO_IsoINIncomplete(USBD_HandleTypeDef *pdev, uint8_t epnum) {
  if (epnum == (AUDIO_EPIN_ADDR & 0x0FU)) {
    USBD_LL_FlushEP(pdev, AUDIO_EPIN_ADDR);
  }
...
static uint8_t USBD_AUDIO_IsoOUTIncomplete(USBD_HandleTypeDef *pdev, uint8_t epnum) {
  ...
  if (epnum == (AUDIO_EPOUT_ADDR & 0x0FU)) {
    haudio = (USBD_AUDIO_HandleTypeDef *)pdev->pClassDataCmsit[pdev->classId];
    if (haudio == NULL) {
      return (uint8_t)USBD_FAIL;
    }
